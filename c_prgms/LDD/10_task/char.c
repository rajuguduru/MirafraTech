...
static ssize_t my_read(struct file *filp, char __user *buf, size_t len, loff_t *off)
{
	copy_to_user(buf,kbuff[++j],len); 
	printk("Copied %d into user buffer from kernel=%s\n",j,kbuff[j]);  
	if(j==2)
	{
		i=-1;
		j=-1;	
...
static ssize_t my_write(struct file *filp, const char __user *buf, size_t len, loff_t *off)
{
	...
	copy_from_user(kbuff[++i],buf,len); 
	printk("Copied %d into kernel buffer from user=%s\n",i,kbuff[i]);  
        return 0;
}
